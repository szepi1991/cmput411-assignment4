...
void main() {
    ...
    vec4 grass = texture2D(textureSample_1,gl_TexCoord[0].st*15.0); // only gl_TexCoord[0] is defined yet.. change
    float alpha = texture2D(textureSample_1,gl_TexCoord[0].st).a; // only gl_TexCoord[0] is defined yet.. change
